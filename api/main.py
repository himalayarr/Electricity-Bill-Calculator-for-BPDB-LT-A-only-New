...
def index():
    bill = None
    breakdown = []
    rate_type = "new" # Default is 'new' for initial page load
    unit = None # Variable to keep track of the inputted unit

    if request.method == "POST":
        # Fetch the selected rate type FIRST
        rate_type = request.form.get("rate_type", "new")
        
        try:
            unit_input = request.form.get("unit")
            if unit_input:
                unit = int(unit_input)
                temp_unit = unit # We use temp_unit for calculation to preserve the original 'unit'
    ...
    # Pass 'unit' back to the template to persist the input value and display it
    return render_template("index.html", bill=bill, breakdown=breakdown, rate_type=rate_type, unit=unit, year=datetime.now().year)
